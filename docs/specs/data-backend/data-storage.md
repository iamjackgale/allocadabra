...
Define how CoinGecko market data is stored in browser-local cache for the app and updated over time.

## Storage Boundary

- All CoinGecko market-data storage is local to the user's browser.
- The app should not require a separate backend service for market-data persistence.
- CoinGecko cache data should persist until the user clears browser storage/cache outside the app.
...
- On app load, check browser cache for an existing token list.
...
- Before fetching, check browser cache for existing price data for each selected token ID.
...
- CoinGecko data should be treated as a growing browser-local cache.
...
- CoinGecko data is lost only if the user clears browser storage/cache outside the app.
...
- Exact browser storage technology: IndexedDB is the likely default because token price history can grow beyond simple localStorage limits.
